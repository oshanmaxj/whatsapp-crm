...
module.exports = { async up(q, S) {
  ...
  const tables=(await q.showAllTables()).map(value=>String(value).toLowerCase());
  if(!tables.includes('call_activities'))await q.createTable('call_activities',{
  ...
  if(!tables.includes('lead_status_history'))await q.createTable('lead_status_history',{
  ...
  if(!tables.includes('conversion_attributions'))await q.createTable('conversion_attributions',{
  ...
  for(const [t,f,n,u] of [['call_activities',['agent_user_id','started_at'],'call_agent_started_idx'],['call_activities',['lead_id','started_at'],'call_lead_started_idx'],['lead_status_history',['to_status_id','changed_at'],'lead_status_changed_idx'],['conversion_attributions',['attribution_key'],'conversion_attribution_key_uq',true]]){
    const indexes=await q.showIndex(t);
    if(!indexes.some(index=>index.name===n))await q.addIndex(t,f,{name:n,unique:!!u});
  }
}, async down(){} };

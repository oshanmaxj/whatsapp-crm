...
exports.followups=async(q,r,n)=>{try{const page=Math.max(1,+q.query.page||1),pageSize=Math.min(100,Math.max(1,+q.query.pageSize||20)),where={};if(q.query.status)where.status=q.query.status;if(!q.user.isSystemAdmin&&!q.user.permissions?.includes('call_center.view_all_agents'))where.assignedTo=q.user.id;const result=await require('../models').Followup.findAndCountAll({where,include:[{model:require('../models').Lead,as:'lead',include:[{model:require('../models').Contact,as:'contact'},{model:require('../models').LeadStatus,as:'status'}]},{model:require('../models').User,as:'assignee',attributes:['id','firstName','lastName','email']}],order:[['due_date','ASC']],limit:pageSize,offset:(page-1)*pageSize,distinct:true});return ok(r,{items:result.rows,pagination:{page,pageSize,total:result.count,totalPages:Math.ceil(result.count/pageSize)}})}catch(e){n(e)}};

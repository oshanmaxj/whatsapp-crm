...
export default function FlowNodeConfigDialog({ node, open, onClose, onSave, onDelete, departments = [], users = [], actionOptions = {}, actionOptionsLoading = false, actionOptionsError = '', onRefreshActionOptions, onLabelOptionsChange, flowId, whatsappAccountId }) {
  ...
  if (type === 'start') {
    form = <><Section title="Flow trigger">
      ...
      {field('source', 'Start flow from', { select: true, children: ['inbound_message', 'any_message', 'first_message', 'button_reply', 'list_reply', 'template_button_reply', 'payment_event', 'label_added', 'contact_created', 'lead_status_changed', 'campaign_response', 'manual', 'facebook_message_received', 'facebook_comment_received', 'facebook_comment_keyword'].map((value) => <MenuItem key={value} value={value}>{value.replaceAll('_', ' ')}</MenuItem>) })}

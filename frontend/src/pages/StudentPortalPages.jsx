...
import {
  ...
  Container, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Divider, LinearProgress, MenuItem, Paper, Stack, TextField, Toolbar, Typography,
...
export class StudentPortalErrorBoundary extends React.Component {
  constructor(props) { super(props); this.state = { failed: false }; }
  static getDerivedStateFromError() { return { failed: true }; }
  componentDidCatch(error, info) {
    console.error('student_portal_page_render_failed', {
      errorName: error?.name || 'Error',
      message: String(error?.message || 'Unknown render error').slice(0, 240),
      componentStack: String(info?.componentStack || '').slice(0, 1200)
    });
  }
  render() {
    if (!this.state.failed) return this.props.children;
    return <Paper variant="outlined" sx={{ p: 3, maxWidth: 640, mx: 'auto' }}>
      <Typography variant="h5" fontWeight={900}>This page could not be displayed</Typography>
      <Typography color="text.secondary" sx={{ my: 1.5 }}>Your session is still active. Retry the page or return to the dashboard.</Typography>
      <Stack direction="row" spacing={1}><Button variant="contained" onClick={() => this.setState({ failed: false })}>Retry</Button><Button component={Link} to="/student/dashboard">Dashboard</Button></Stack>
    </Paper>;
  }
}
...
export function StudentPortalLayout() {
  ...
  return <Box sx={{ minHeight: '100vh', bgcolor: '#f5f8f7', pb: { xs: 9, sm: 3 } }}>
    ...
    <Container maxWidth="lg" sx={{ py: 3 }}><StudentPortalErrorBoundary><Outlet /></StudentPortalErrorBoundary></Container>
...
export function StudentSupportTicketsPage() {
  const [tickets,setTickets]=useState([]),[categories,setCategories]=useState([]),[selected,setSelected]=useState(null),[creating,setCreating]=useState(false),[loading,setLoading]=useState(true),[form,setForm]=useState({categoryId:'',enrollmentId:'',subject:'',description:''}),[reply,setReply]=useState(''),[busy,setBusy]=useState(false),[notice,setNotice]=useState(''),[error,setError]=useState('');
  const load=async({silent=false}={})=>{try{if(!silent)setLoading(true);setError('');const[t,c]=await Promise.all([listStudentSupportTickets(),getStudentSupportCategories()]);setTickets(t.data.data.items||[]);setCategories(c.data.data||[]);}catch(e){console.warn('student_support_tickets_load_failed',{code:e.response?.data?.code||'REQUEST_FAILED',status:e.response?.status||null});setError(e.response?.data?.message||'Unable to load support tickets. Please try again.');}finally{if(!silent)setLoading(false);}};
  useEffect(()=>{load();const timer=window.setInterval(()=>load({silent:true}),30000);return()=>window.clearInterval(timer);},[]);
  const create=async()=>{if(!form.categoryId||!form.subject.trim()||!form.description.trim()){setError('Category, subject and description are required.');return false;}try{setBusy(true);setError('');const r=await createStudentSupportTicket(form);setNotice(`Your support request has been created. Ticket: ${r.data.data.ticketNumber}`);setForm({categoryId:'',enrollmentId:'',subject:'',description:''});await load();return true;}catch(e){setError(e.response?.data?.message||'Unable to create support request.');return false;}finally{setBusy(false);}};
  ...
  return <Stack spacing={2.5} sx={{ml:{sm:20}}}><Stack direction="row" justifyContent="space-between" alignItems="center"><Box><Typography variant="h3" fontWeight={950}>Support Tickets</Typography><Typography color="text.secondary">Report a problem and follow its progress.</Typography></Box><Button variant="contained" onClick={()=>setCreating(true)}>Create ticket</Button></Stack>{notice&&<Alert severity="success">{notice}</Alert>}{error&&<Alert severity="error" action={<Button color="inherit" onClick={()=>load()}>Retry</Button>}>{error}</Alert>}{loading?<PageLoading/>:<><Typography variant="h5" fontWeight={900}>My tickets</Typography>{tickets.map(ticket=><Paper key={ticket.id} component="button" onClick={()=>open(ticket)} variant="outlined" sx={{p:2,textAlign:'left',bgcolor:'background.paper',border:'1px solid',borderColor:'divider',cursor:'pointer'}}><Stack direction="row" justifyContent="space-between"><Box><Typography fontWeight={900}>{ticket.ticketNumber}</Typography><Typography>{ticket.subject}</Typography><Typography variant="caption" color="text.secondary">Updated {new Date(ticket.updatedAt).toLocaleString()}</Typography></Box><Chip label={ticket.status.replaceAll('_',' ')}/></Stack></Paper>)}{tickets.length===0&&<Paper variant="outlined" sx={{p:4,textAlign:'center'}}><Typography fontWeight={800}>No support tickets yet</Typography><Typography color="text.secondary">Create a ticket when you need help.</Typography></Paper>}</>}
  <Dialog open={creating} onClose={()=>!busy&&setCreating(false)} fullWidth><DialogTitle>Create support request</DialogTitle><DialogContent><DialogContentText sx={{mb:2}}>Describe the problem clearly so the support team can investigate it.</DialogContentText><Stack spacing={2}><TextField select required label="Category *" value={form.categoryId} onChange={e=>setForm({...form,categoryId:e.target.value})}>{categories.map(c=><MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>)}</TextField><TextField required label="Subject *" value={form.subject} onChange={e=>setForm({...form,subject:e.target.value})} inputProps={{maxLength:240}}/><TextField required multiline minRows={4} label="Detailed description *" value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/></Stack></DialogContent><DialogActions><Button disabled={busy} onClick={()=>setCreating(false)}>Cancel</Button><Button variant="contained" disabled={busy} onClick={async()=>{if(await create())setCreating(false);}}>{busy?<CircularProgress size={20}/>: 'Create ticket'}</Button></DialogActions></Dialog>
...
export function StudentCoursePage() {
  ...
  if (!data && !error) return <PageLoading />;
  if (error) return <Alert severity="error">{error}</Alert>;
  const topics = data.topics || data.course?.topics || [];
  return <Stack spacing={2.5} sx={{ ml: { sm: 20 } }}><Box><Button component={Link} to="/student/courses">← My courses</Button><Typography variant="h3" fontWeight={950}>{data.course.name}</Typography><Typography color="text.secondary">{data.course.shortDescription || data.course.description}</Typography></Box><Paper variant="outlined" sx={{ p: 2 }}><Stack direction="row" justifyContent="space-between"><Typography fontWeight={850}>Course progress</Typography><Typography>{data.progress.percentage}%</Typography></Stack><LinearProgress variant="determinate" value={data.progress.percentage} sx={{ mt: 1 }} /><Typography variant="caption">{data.progress.completedLessons} of {data.progress.totalLessons} lessons complete</Typography></Paper>{!data.enrollmentAccess?.allowed && <Alert severity="warning">{data.enrollmentAccess?.warning || paymentWarning}</Alert>}{topics.map((topic, topicIndex) => { const completed = topic.lessons.filter((lesson) => lesson.progress?.isCompleted).length; return <Accordion defaultExpanded={topicIndex === 0} key={topic.id} disableGutters><AccordionSummary expandIcon={<ExpandMoreIcon />}><Box sx={{ flex: 1 }}><Stack direction="row" justifyContent="space-between" alignItems="center"><Typography variant="h6" fontWeight={900}>{topic.title}</Typography><Chip size="small" label={`${topic.lessons.length} lesson${topic.lessons.length===1?'':'s'}`} /></Stack><Typography variant="body2" color="text.secondary">{topic.summary || `${completed} complete`}</Typography></Box></AccordionSummary><AccordionDetails sx={{ p: 0 }}><Stack divider={<Divider />}>{topic.lessons.map((lesson, index) => <Box key={lesson.id} sx={{ p: 2, display: 'flex', alignItems: 'center', gap: 2 }}><Chip size="small" label={lesson.progress?.isCompleted ? '✓' : index + 1} color={lesson.progress?.isCompleted ? 'success' : 'default'} /><Box sx={{ flex: 1 }}><Typography fontWeight={750}>{lesson.title}</Typography><Typography variant="caption" color="text.secondary">{lesson.lessonType} {lesson.durationMinutes ? `· ${lesson.durationMinutes} min` : ''}{lesson.releaseAt ? ` · Releases ${new Date(lesson.releaseAt).toLocaleString()}` : ''}</Typography></Box>{lesson.locked || lesson.accessStatus === 'payment_blocked' ? <Chip color={lesson.lockReason==='scheduled_release'?'info':'default'} label={lesson.lockReason==='scheduled_release'?'Scheduled':'Locked'} /> : <Button component={Link} to={`/student/lessons/${lesson.id}`} variant="outlined" size="small">Open</Button>}</Box>)}{!topic.lessons.length && <Box sx={{ p: 2 }}><Typography color="text.secondary">No published lessons in this topic yet.</Typography></Box>}</Stack></AccordionDetails></Accordion>; })}{!topics.length && <Paper variant="outlined" sx={{ p: 4 }}><Typography color="text.secondary">No published topics are available yet.</Typography></Paper>}</Stack>;
}
